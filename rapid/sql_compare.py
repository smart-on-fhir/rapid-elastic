...
def union_views(create: str, table_list: List[str], create_table=False, alias_col='alias') -> str:
    if create_table:
...
def union_views_file(create: str, table_list: List[str], create_table=False, alias_col='alias') -> Path:
    return filetool.write_text(
        contents=union_views(create, table_list, create_table, alias_col),
